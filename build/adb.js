...
export async function checkAdb() {
    try {
        ...
        throw new Error('adb コマンドが見つかりません。Android SDK Platform-Tools がインストールされ、PATHが通っていることを確認してください。');
    }
}
/**
 * Executes a general adb command and returns the stdout.
 */
export async function adbExec(command) {
    try {
 ...
 */
export async function getUiHierarchy() {
    try {
